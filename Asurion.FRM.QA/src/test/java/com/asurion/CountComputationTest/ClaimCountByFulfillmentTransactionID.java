...
public class ClaimCountByFulfillmentTransactionID {
	@Test
	public void ClaimCountByFulfillmentTransactionID() throws SQLException {
		...
		try {
			...
			myRs = myStmt.executeQuery("SELECT FRAUD_ACTION_CODE, DECISION_RULE_ID, DECISION_TABLE_NAME FROM QAFRMRTM.FRAUD_ACTION where SERVICE_REQUEST_NBR = '312312312345'and DECISION_TABLE_NAME = 'ClaimCountByFulfillmentTransactionID';");
			...
			while (myRs.next()) {
				...
				Assert.assertEquals(RuleName, "ClaimCountByFulfillmentTransactionID");
				
				String ActionCode = myRs.getString("FRAUD_ACTION_CODE");
				Assert.assertEquals(ActionCode, "NO E-CARD");
	
				int RuleId = myRs.getInt("DECISION_RULE_ID");
				Assert.assertEquals(RuleId, 5);
