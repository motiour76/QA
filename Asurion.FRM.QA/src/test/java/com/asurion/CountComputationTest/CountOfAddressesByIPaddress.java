...
public class CountOfAddressesByIPaddress {

	@Test
	public void CountOfAddressesByIPaddress() throws SQLException {
		...
		try {
			...
			myRs = myStmt.executeQuery("SELECT FRAUD_ACTION_CODE, DECISION_RULE_ID, DECISION_TABLE_NAME FROM QAFRMRTM.FRAUD_ACTION where SERVICE_REQUEST_NBR = '2721231123' and DECISION_TABLE_NAME = 'CountOfAddressesByIPaddress';");
			...
			while (myRs.next()) {
				...
				Assert.assertEquals(RuleName, "CountOfAddressesByIPaddress");
				
				String ActionCode = myRs.getString("FRAUD_ACTION_CODE");
				Assert.assertEquals(ActionCode, "NO PORTAL");
	
				int RuleId = myRs.getInt("DECISION_RULE_ID");
				Assert.assertEquals(RuleId, 6);
