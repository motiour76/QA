...
public class CustomersByIPaddress {
	@Test
	public void CustomersByIPaddress() throws SQLException {
		...
		try {
			...
			myRs = myStmt.executeQuery("SELECT FRAUD_ACTION_CODE, DECISION_RULE_ID, DECISION_TABLE_NAME FROM QAFRMRTM.FRAUD_ACTION where SERVICE_REQUEST_NBR = '1432141331121' and DECISION_TABLE_NAME = 'CustomersByIPaddress';");		
			...
			while (myRs.next()) {
				...
				Assert.assertEquals(RuleName, "CustomersByIPaddress");
